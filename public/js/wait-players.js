...
async function loadGameSettings() {
    try {
        const response = await autoReauthOn401(() => fetch(`/api/settings/${gameState.roomId}`, { headers: getAuthHeaders() }));
...
async function updateRoom() {
    try {
        const response = await autoReauthOn401(() => fetch(`/api/room/${gameState.roomId}`, { headers: getAuthHeaders() }));
